...
- **Failure.** Failure, abandonment, expiry, or presenting an already-`CLOSED` account leaves the account exactly as authoritative state already is — never resurrected from `CLOSED` to `DISABLED`, and never advanced to `ACTIVE` by an attempt that did not itself commit. There is no partial success and no fallback path. This attempt's own `DISABLED` → `ACTIVE` transition did not commit, so it consumes nothing itself and creates no replacement obligation of its own; whether the credential it presented is observed as valid or as already consumed depends on authoritative state, not on this attempt's failure alone (`LOCAL_FAILURE_IMPLIES_GLOBAL_STATE_UNCHANGED=no`) — it **remains valid** only where no other commit has already consumed it, and a credential some other attempt's successful commit already consumed is not restored to valid merely because this attempt failed (§11.2's commit-point and authoritative-state rules).
...
47. **Concurrent commit attempts on one credential** — for two or more concurrent attempts presenting the same single-use recovery credential to reach a committing transition, assert that at most one attempt's transition commits, that every other concurrent or subsequently replayed attempt is denied as an invalid credential, and that this holds even when more than one attempt passes verification before either commits (`RECOVERY_CREDENTIAL_SINGLE_COMMIT_SEMANTICS=yes`, `CONCURRENT_SUCCESSFUL_REPLAY_ALLOWED=no`, §11.2). Assert further that the losing attempt(s) — the one(s) whose transition does not commit — report or reconcile against the winner's committed authoritative state rather than their own pre-race snapshot: a losing attempt must not restore `DISABLED`, must not restore credential validity, and must not create a replacement obligation, a session, or any new authority, regardless of how far its own verification proceeded (`CONCURRENT_LOSER_MAY_RESTORE_DISABLED=no`, `CONCURRENT_LOSER_MAY_RESTORE_CREDENTIAL_VALIDITY=no`, `CONCURRENT_LOSER_CREATES_REPLACEMENT_OBLIGATION=no`, `CONCURRENT_LOSER_IS_NON_AUTHORIZING=yes`).
