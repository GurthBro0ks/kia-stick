...
- **Failure.** Failure, abandonment, expiry or a `CLOSED` account leaves the account in the state it was already in. There is no partial success and no fallback path. Because the `DISABLED` → `ACTIVE` transition did not commit, no recovery-credential consumption occurred: any credential presented **remains valid**, and no replacement obligation is created by the failed attempt (§11.2's commit-point rule).
...
- **Failure.** Failure, abandonment, expiry, or an already-`CLOSED` account leaves the account in the state it was already in. There is no partial success, no fallback path, and no partially-closed state. Because the `DISABLED` → `CLOSED` transition did not commit, no recovery-credential consumption occurred: any credential presented **remains valid**, the account **remains `DISABLED`**, and **no replacement obligation is created by the failed attempt** (`FAILED_PRECOMMIT_CLOSURE_CONSUMES_RECOVERY_CREDENTIAL=no`, `FAILED_CLOSURE_RECOVERY_AUTHORITY_SAFE_STATE_DEFINED=yes`) — resolving `W3_ABORTED_CLOSURE_REPLACEMENT_AUTHORITY_UNDEFINED` without inventing any continuation authority beyond the ceremony's existing bounds. The operation may generate a content-free failure/audit event and remains subject to §15's rate and abuse limits; it must not issue a replacement merely because verification occurred, must not consume the credential merely because the ceremony was entered, must not create a session, and must not reactivate the account (§11.2).
- It is rate-limited at least as strictly as login and recovery (§15), audited content-free (§16), and surfaced to the owner.
...
The next step is independent QA of the exact commit introducing this revision, followed by operator review of the §26 register's six decisions (D1-D6), and then a separate owner authorization decision about whether any later gate begins. This revision repairs `W3_ABORTED_CLOSURE_REPLACEMENT_AUTHORITY_UNDEFINED`, the one remaining material finding independent review raised against revision 6; it does not accept the design.
