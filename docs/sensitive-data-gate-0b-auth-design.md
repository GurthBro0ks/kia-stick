...
REVISION=9 — bounded revision resolving four remaining findings (F1-F4) of the final independent re-QA of revision 8 (commit `8db5b069052b1084b0d03cb6d195fe7f090157a0`, `RESULT=WARN`): **F1** — the transition-table rows for both ceremonies' failure branches (`REACTIVATION_CEREMONY`→`DISABLED`, `ACCOUNT_CLOSURE_CEREMONY`→`DISABLED`) still read "the account is left exactly as it was" unconditionally, contradicting the correctly-conditioned prose next to them and §11.2's authoritative-state rule; both rows now state that the postcondition is governed by authoritative state, not asserted unconditionally. **F2** — the closure ceremony's `Failure` clause checked only for a competing **closure** having already committed, missing the case where a competing **reactivation** on the same credential commits first, leaving `ACTIVE`/consumed authoritative with no stated postcondition for the losing closure attempt; both `Failure` clauses (closure, and symmetrically reactivation) now name every competing transition away from `DISABLED` — not only the same-ceremony case — and state explicitly that a loser must not restore `DISABLED`, must not close or reactivate the account merely because it lost, and must not undo the winning transition. **F3** — test 47's oracle unconditionally required every losing attempt to reconcile against a winner's committed state, which has no referent when **zero** attempts commit, a real required trace test 45 already handled correctly for one non-committing attempt but test 47 did not name for the concurrent case; test 47 is now split into its two exhaustive cases (exactly one commit; zero commits), test 12's "exactly one success" is corrected to `AT_MOST_ONE_SUCCESSFUL_COMMIT`, and test 45 gains a cross-reference making the zero-commit correspondence explicit. **F4** — a validation-evidence and proof-method finding, not a design-content defect: it concerns how a past revision's own validation *procedure* was labeled, and this document makes no validation-mode claim of its own, so it is corrected in `claude-progress.md`'s ledger rather than here. This revision does not reopen the commit point, the outcome-aware replacement rule, the single-commit invariant's substance, or any owner decision: `OWNER_DECISION_COUNT=6` with `OWNER_DECISIONS_ACCEPTED=0`, D1-D6 untouched, no D7 created. Resolves `W5_CLOSURE_REACTIVATION_RACE_AND_ZERO_COMMIT_ORACLE_GAPS`. Only the two §17.1 transition-table rows, the two §17.1 ceremony `Failure` clauses, §21 THREAT-020/THREAT-021, and §22 tests 12, 45 and 47 are revised. REVISION=8 was the bounded single-finding revision resolving `W4_NONCOMMIT_FAILURE_POSTCONDITIONS_CONFLICT_WITH_COMMITTED_STATE`, the one remaining material finding of the independent re-QA of revision 7 (commit `59f65734b0d18a3b309f8b7a716dd11c2209b311`, `RESULT=WARN`): revision 7 correctly fixed the commit point itself — verification and ceremony entry are not consumption, only a successful commit consumes, and a ceremony that never commits consumes nothing of its own — but its failure-postcondition wording then stated or implied that a non-committing attempt *always* leaves the account `DISABLED` and any presented credential `VALID`, without conditioning that pair on authoritative state. That is the correct postcondition only where no other operation has already committed a transition on the same account or credential. It is wrong where a concurrent attempt already committed `DISABLED` → `CLOSED` first and consumed the credential, and wrong where the attempt is a retry against an account already authoritatively `CLOSED`: §17.1 line 983 named an already-`CLOSED` account inside its own `Failure` branch and then promised `DISABLED`/valid regardless, and §22 tests 44/45 and 46/47 demanded incompatible outcomes on the same concurrent trace. This revision does not reopen the commit point, the consumption rule, or the outcome-aware replacement rule from revisions 6-7 — it makes the **failure postcondition** conditional on **authoritative current state** instead of on the failing attempt's own snapshot: a non-committing attempt never mutates or rolls back authoritative state itself (`NONCOMMITTING_ATTEMPT_MAY_NOT_CHANGE_OR_ROLL_BACK_AUTHORITATIVE_STATE=yes`), and what is actually observed afterwards is read from authoritative state rather than asserted independently of it (`FAILURE_POSTCONDITION_IS_DERIVED_FROM_AUTHORITATIVE_CURRENT_STATE=yes`) — `DISABLED`/valid absent a competing commit; whatever a winning concurrent commit already left it as, for a losing attempt; and `CLOSED`/consumed, non-authorizing, for a retry against an already-closed account. That one attempt did not commit is not the same fact as global state being unchanged (`LOCAL_FAILURE_IMPLIES_GLOBAL_STATE_UNCHANGED=no`), because another attempt may have committed instead. This is a failure-postcondition precision, **not** a new owner decision and **not** a reversal of the single-commit, no-replay, or post-commit response-loss invariants revision 7 already established — it restates them from the losing/retry side rather than contradicting them: `OWNER_DECISION_COUNT=6` with `OWNER_DECISIONS_ACCEPTED=0`, D1-D6 untouched, no D7 created. Only §11.2 (one clause narrowed, one bullet added), the two §17.1 ceremony `Failure` clauses, §21 THREAT-020/THREAT-021, and §22 tests 44, 45, 47 plus one added test (48) are revised. REVISION=7 was the bounded single-finding revision resolving `W3_ABORTED_CLOSURE_REPLACEMENT_AUTHORITY_UNDEFINED`, the one remaining material finding of the independent re-QA of revision 6 (commit `4d161cac6fb9489139fbdc6b7b50e508d87f03c2`, `RESULT=WARN`): revision 6 made the §11.2 consumption rule outcome-aware but left one gap — a disabled-account closure ceremony may verify and use the accepted recovery authority, then fail, be abandoned, or expire before its `DISABLED` → `CLOSED` transition commits, and §11.2's closing bullet then read as leaving the credential consumed, the account `DISABLED`, and a replacement obligation standing with no authority capable of discharging it. This revision closes that gap by defining precisely **when** a credential is consumed rather than by inventing any new continuation authority. `RECOVERY_CREDENTIAL_CONSUMPTION_COMMIT_POINT=SUCCESSFUL_COMMIT_OF_THE_AUTHORIZED_RECOVERY_OR_ACCOUNT_STATE_TRANSITION`: presenting a recovery credential is not consumption, verifying it is not consumption, and being admitted to a reactivation or closure ceremony is not consumption (`RECOVERY_CREDENTIAL_VERIFICATION_EQUALS_CONSUMPTION=no`, `CEREMONY_ENTRY_EQUALS_CONSUMPTION=no`); a credential is consumed only when its authorized transition reaches a successful commit (`SUCCESSFUL_COMMIT_CONSUMES_CREDENTIAL=yes`). Consequently a ceremony that fails, is abandoned, or expires before its transition commits has consumed nothing at all — the credential remains valid and no replacement obligation is created by that attempt (`FAILED_PRECOMMIT_CLOSURE_CONSUMES_RECOVERY_CREDENTIAL=no`, `STRANDED_DISABLED_CONSUMED_CREDENTIAL_STATE_ALLOWED=no`) — which is a narrower, more precise statement of the existing rule, not an exception to it. This revision also states, as future implementation requirements rather than as anything designed here, that the affected account-state transition and the credential consumption must commit as one atomic unit (`CLOSURE_STATE_AND_CREDENTIAL_CONSUMPTION_ATOMIC=yes`), that a response lost after a successful commit must not permit a retry to roll back state, re-run the transition, or mint new authority (`POST_COMMIT_RESPONSE_FAILURE_DOES_NOT_ROLL_BACK_SECURITY_STATE=yes`, `CLOSURE_RETRY_AFTER_COMMIT_IS_NON_AUTHORIZING=yes`), and that concurrent attempts on one single-use credential may commit at most once (`RECOVERY_CREDENTIAL_SINGLE_COMMIT_SEMANTICS=yes`, `CONCURRENT_SUCCESSFUL_REPLAY_ALLOWED=no`); it selects no database, transaction mechanism, or storage engine for any of these. The existing outcome-aware nonterminal/terminal split (below) is unchanged and now applies, unambiguously, only once a transition actually commits. This is a security state-machine and failure-semantics clarification, **not** a new owner decision: `OWNER_DECISION_COUNT=6` with `OWNER_DECISIONS_ACCEPTED=0` and D1-D6 are untouched, no D7 is created, and the closure/Gate 0C boundary is unchanged. Only §11.2, the two §17.1 ceremony `Failure` clauses and one `After success` clause, §21 THREAT-020/THREAT-021, and §22 test 44 plus three added tests are revised. REVISION=6 was the bounded single-finding revision resolving `W2_CLOSURE_REPLACEMENT_LIFECYCLE_CONTRADICTION`, the one remaining material finding of the independent re-QA of revision 5 (commit `bc7de954944771839df61d343d8dc8aeb0ddf497`, `RESULT=WARN`): §11.2's recovery-credential lifecycle stated replacement-on-use **unconditionally**, §17.1's `ACCOUNT_CLOSURE_CEREMONY` applies that lifecycle "in full", and §17.3 and §26.6 nonetheless state that a direct `DISABLED` → `CLOSED` closure issues no replacement credential — three statements that cannot all serve as one unambiguous specification. This revision makes the §11.2 consumption rule **outcome-aware**: a successful use always consumes and permanently invalidates the credential, and what it obliges afterwards follows from the state the ceremony leaves the account in — replacement is **mandatory** where the account retains future authentication or recovery capability (`RECOVERY_CREDENTIAL_USED_FOR_NONTERMINAL_RECOVERY=CONSUME_AND_REPLACE_OR_ROTATE`), and **no replacement is issued** where the ceremony terminally closes the account (`RECOVERY_CREDENTIAL_USED_FOR_TERMINAL_CLOSURE=CONSUME_WITHOUT_REPLACEMENT`), because `CLOSED` has no continuing authority for a replacement to serve. That is a lifecycle consequence of the resulting account state, **not** a new owner decision and **not** an exception permitting reuse: `RECOVERY_CREDENTIAL_REPLAY_AFTER_USE_ALLOWED=no`, and single use, replay rejection, predecessor invalidation, confirmed issuance, audit, no-plaintext-logging and no-operator-recovery are all unweakened. Only §11.2, the two §17.1 "After success" clauses, one §17.3 sentence, §21 THREAT-020/THREAT-021 and §22 tests 12, 37 and 44 are revised; `OWNER_DECISION_COUNT=6` with `OWNER_DECISIONS_ACCEPTED=0`, D1-D6 are untouched, the closure authority model and `ACCOUNT_CLOSURE_CEREMONY` bounds are preserved, and `ACCOUNT_CLOSURE_IMPLIES_PRIVATE_DATA_DELETION=no` / `ACCOUNT_CLOSURE_IMPLIES_KEY_DESTRUCTION=no` / `PRIVATE_DATA_DISPOSITION_AFTER_ACCOUNT_CLOSURE=UNRESOLVED_PENDING_LATER_GATES` are unchanged. REVISION=5 was the bounded single-finding revision resolving `W1_D5_D6_COMPARATIVE_HARM_OVERSTATEMENT`, the one remaining material finding of the independent re-QA of revision 4 (commit `843f005f0c09d347b354996308f087ff8d463e09`, `RESULT=WARN`): §17.3 and §26.6 classified disabled-account self-closure as `OWNER_DECISION_D6` correctly, but justified that classification with a comparative-harm claim — that D6 presents strictly greater and uniquely irreversible harm than D5. That justification does not hold, because under D5(b) a reactivated account can obtain the §11.2 replacement recovery material, reach the credential-reset ceremony, authenticate freshly and close from `ACTIVE`, so D5(b) can also compose into a permanently closed account. The comparative-harm and unique-irreversibility claims are **withdrawn**; D6 is re-justified on **logical independence** and on what it directly authorizes while the account remains `DISABLED`; and §17.3 gains an explicit D5/D6 independence matrix. `D6_IS_ONLY_PATH_TO_PERMANENT_ACCOUNT_CLOSURE=no`; `D6_STRICTLY_GREATER_HARM_THAN_D5=no`. Only §17.3, §26.5 `SECURITY_TRADEOFFS` and §26.6 `SECURITY_TRADEOFFS`/`RECOMMENDATION` are revised: the closure authority model, the `ACCOUNT_CLOSURE_CEREMONY` bounds, the state machine, the register's contents and count, and every area previous review passed are preserved unchanged, and no owner decision is accepted (`OWNER_DECISIONS_ACCEPTED=0`). REVISION=4 was the bounded single-finding revision resolving `S1_DISABLED_CLOSURE_AUTHORITY_UNRECONCILED`, the one remaining material ambiguity of the independent re-QA of revision 3 (commit `297f7343d6d3c1a2c61f7e767e9b1663cf32e24d`, `RESULT=WARN`): §17.1 promised a `DISABLED` → `CLOSED` election while §4.6 required session-bound re-authentication for account closure and `DISABLED_ORDINARY_SESSION_AUTHORITY=invalid`, so the design never named what authority, if any, may enter a closure ceremony from `DISABLED`. This revision separates `ACTIVE_ACCOUNT_CLOSURE_AUTHORITY` from `DISABLED_ACCOUNT_CLOSURE_AUTHORITY`, specifies a narrowly bounded `ACCOUNT_CLOSURE_CEREMONY` for the disabled case, states explicitly that account closure is **not** private-data deletion or key destruction, and registers the underlying product choice as `OWNER_DECISION_D6` (§26.6) rather than deciding it. Only §4.6, §4.9, §9.4, §17, §20, §21, §22, §23 and §26 entries that closure authority directly touches are revised; every area the revision-3 re-QA passed is preserved unchanged. REVISION=3 was the bounded final-blocker revision resolving the three remaining material findings of the independent re-QA of revision 2 (commit `725b8fbc9e80d624ba44954592dce9a624174c83`, `RESULT=FAIL`): R1 the hidden fifth owner decision, now surfaced as `OWNER_DECISION_D5` (§26.5); R2 the incoherent disabled-account reactivation state machine (§9.4, §17); and R3 D3's crossing of the Gate 0C encryption boundary (§26.3). Revision 2 resolved F1-F6 and the F7 clarification against revision 1 (commit `5127132bfd3a6107941118f2d95152d202546db8`); the re-QA confirmed F1, F3, F4, F5 and F7 as resolved, and those areas — together with the MFA qualification boundary, the browser/TLS model, the authorization-bypass model and the validation-equivalence record — are preserved unchanged here. Only R1, R2, R3 and the register/traceability/future-test entries they directly touch are revised.
...
**DESIGN CHOICE.** At account creation the user is issued a high-entropy **recovery credential** — the term used consistently from here on, in place of revision 1's "recovery secret," to keep it visibly a credential subject to §4.8 and §16.4 rather than an incidental string. It is displayed once, stored by the user outside the application (written down, or in a password manager), and stored server-side only as a verifier. Using it authorizes a credential-reset ceremony — not an ordinary session (§4.8) — is single-use, and on consumption is replaced according to the **outcome-aware** rule below: replacement is mandatory wherever the ceremony leaves the account with future authentication or recovery capability, and is neither issued nor owed where the ceremony terminally closes the account. Its full lifecycle is below, and holds under every `OWNER_DECISION_D4` outcome.
...
- **Consumption occurs only at successful commit — never at presentation, verification, or ceremony entry.** `RECOVERY_CREDENTIAL_CONSUMPTION_COMMIT_POINT=SUCCESSFUL_COMMIT_OF_THE_AUTHORIZED_RECOVERY_OR_ACCOUNT_STATE_TRANSITION`. Presenting the credential to a ceremony is not consumption; the ceremony verifying it is not consumption; being admitted to a limited recovery, reactivation, or closure ceremony is not consumption (`RECOVERY_CREDENTIAL_VERIFICATION_EQUALS_CONSUMPTION=no`, `CEREMONY_ENTRY_EQUALS_CONSUMPTION=no`). None of this loosens what the ceremony may do before that point — it remains exactly as narrowly authorized as §17.1 states for whichever ceremony it is, and successful verification never itself grants private-data authority, an ordinary session, or any capability beyond the ceremony's own bounds. A credential is consumed only when the specific transition it authorizes — the ordinary credential-reset ceremony's replacement (§11.1), a `DISABLED` → `ACTIVE` reactivation, or a `DISABLED` → `CLOSED` closure (§17.1) — reaches its own successful commit (`SUCCESSFUL_COMMIT_CONSUMES_CREDENTIAL=yes`). Everywhere else in this document, "successful use" of a recovery credential means exactly this — a credential-authenticated operation whose authorized state transition reached its successful commit point — and never mere presentation or verification.
- **Use is one-time.** A successful consumption permanently invalidates that credential. `EXACTLY_ONE_SUCCESSFUL_CONSUMPTION`; `RECOVERY_CREDENTIAL_SINGLE_COMMIT_SEMANTICS=yes` — where two ceremonies race, at most one attempt's transition **commits**, and every other concurrent or later attempt is denied as an invalid credential rather than proceeding (`CONCURRENT_SUCCESSFUL_REPLAY_ALLOWED=no`); this is a guarantee about the commit itself, not merely about the verification check that precedes it, so two attempts that both pass verification still cannot both commit. Replay of a consumed credential fails closed and emits an audit event (§22 test 29). This document selects no database, transaction mechanism, or storage engine to provide that guarantee — only the invariant it must satisfy.
- **Use consumes the credential; what consumption *obliges* depends on the resulting account state.** `RECOVERY_LIFECYCLE_OUTCOME_AWARE=yes`. A successful consumption always and permanently invalidates the credential, under every outcome (above). What it obliges afterwards is determined by the state the ceremony leaves the account in — not by the ceremony's name and not by who completed it. Revision 5 stated this rule unconditionally ("consumption mandates issuance of a replacement"), which was correct for every ceremony leaving the account usable but contradicted the terminal-closure case §17.1 specifies and §17.3 and §26.6 describe; independent re-QA raised that contradiction as `W2_CLOSURE_REPLACEMENT_LIFECYCLE_CONTRADICTION`, and it is repaired here by making the rule outcome-aware rather than by carving out any exception to single use, replay rejection, predecessor invalidation, confirmed issuance or audit.
  - **Nonterminal outcome — consumption mandates replacement.** `RECOVERY_CREDENTIAL_USED_FOR_NONTERMINAL_RECOVERY=CONSUME_AND_REPLACE_OR_ROTATE`. Where the ceremony leaves the account in a state retaining any future ordinary authentication or recovery capability — the ordinary credential-reset ceremony (§11.1), or a `DISABLED` → `ACTIVE` reactivation under D5(b) (§17.1) — consumption **mandates** issuance of a replacement through the same confirmed-issuance rule, and continuing recovery capability is not to be treated as available until that issuance is confirmed. If replacement issuance fails or is not confirmed, the account is left in a defined state — the consumed credential remains invalid, no new credential is silently minted, and the outstanding replacement obligation is recorded and surfaced to the user — rather than in an ambiguous one. A replacement is recovery material and never becomes an ordinary login credential (§4.8, §11.1).
  - **Terminal outcome — consumption without replacement.** `RECOVERY_CREDENTIAL_USED_FOR_TERMINAL_CLOSURE=CONSUME_WITHOUT_REPLACEMENT`. Where the ceremony's success **terminally closes** the account — the `DISABLED` → `CLOSED` transition of §17.1's `ACCOUNT_CLOSURE_CEREMONY`, if `OWNER_DECISION_D6` (§26.6) is ever answered (b) — the credential is consumed and permanently invalidated and **no replacement recovery credential is issued**. No outstanding replacement obligation arises, is recorded, or is surfaced, because none is owed. The reason is terminality, not convenience: `CLOSED` is terminal under Gate 0B (`CLOSED_STATE_TERMINAL_FOR_GATE_0B=yes`, `CLOSED_REACTIVATION_ALLOWED=no`), a closed account has no continuing ordinary authentication or recovery authority for a replacement to serve, and issuing one would manufacture exactly the residual access authority closure exists to end. This is **not** an exception permitting reuse and weakens nothing above: the consumed credential is as invalid here as anywhere, replay fails closed and is audited (§22 tests 29, 44), no ordinary session is created, and no new authentication or recovery authority of any kind is created. It is likewise **not** a data or key operation — consuming a credential without replacing it deletes no private content, destroys no keys, performs no cryptographic erasure or backup eradication, and touches nothing Gate 0C owns (`ACCOUNT_CLOSURE_IMPLIES_PRIVATE_DATA_DELETION=no`, `ACCOUNT_CLOSURE_IMPLIES_KEY_DESTRUCTION=no`, §17.1, §20).
  - **Only an actual terminal transition is terminal, and only an actual commit is consumption.** The nonterminal and terminal rules above govern a transition that **commits**; the terminal rule is conditioned on the `DISABLED` → `CLOSED` transition having actually occurred, never on the ceremony merely having been entered. A closure or reactivation ceremony that is abandoned, times out, expires, fails validation, or hits a pre-commit application or system error before its transition commits has not reached the consumption point defined above, regardless of how far verification or the ceremony otherwise proceeded: that failed attempt does not itself invalidate the credential, does not itself change the account state, and **no replacement obligation is created by that failed attempt**, because none is owed — nothing was consumed by it (`FAILED_PRECOMMIT_CLOSURE_CONSUMES_RECOVERY_CREDENTIAL=no`, `STRANDED_DISABLED_CONSUMED_CREDENTIAL_STATE_ALLOWED=no`). What credential and account state are actually observed afterwards is governed by authoritative state exactly as the next bullet defines, and is **not** asserted here as an unconditional `DISABLED`/valid pair (§17.1's ceremony `Failure` clauses). This corrects revision 6's wording that such a ceremony "falls under the nonterminal rule" and that "if the credential was nonetheless consumed, the replacement obligation stands" — independent re-QA correctly read that as permitting exactly the state this rule forecloses: a `DISABLED` account with a consumed credential, a standing replacement obligation, and no authority able to discharge it (`W3_ABORTED_CLOSURE_REPLACEMENT_AUTHORITY_UNDEFINED`). Under the commit-point rule that state cannot arise, because consumption and the nonterminal replacement obligation are both downstream of a successful commit, and a ceremony that never commits triggers neither. Under every outcome — terminal, nonterminal, or non-committing — `RECOVERY_CREDENTIAL_REPLAY_AFTER_USE_ALLOWED=no`.
  - **A non-committing attempt's postcondition is read from authoritative state, not asserted independently of it.** `NONCOMMITTING_ATTEMPT_MAY_NOT_CHANGE_OR_ROLL_BACK_AUTHORITATIVE_STATE=yes`; `FAILURE_POSTCONDITION_IS_DERIVED_FROM_AUTHORITATIVE_CURRENT_STATE=yes`; `NONCOMMITTING_ATTEMPT_MUTATES_AUTH_STATE=no`; `NONCOMMITTING_ATTEMPT_MAY_ROLL_BACK_CONCURRENT_COMMIT=no`. The bullet above establishes what a ceremony that never commits does **not** do — it does not itself consume the credential, does not itself create a replacement obligation, does not itself create a session, and does not itself create private-data authority. It does not establish that the account and credential remain in whatever state that attempt happened to observe when it began, because another attempt may have committed in the interim: that this attempt did not commit (`LOCAL_ATTEMPT_COMMIT_RESULT`, `THAT_ATTEMPT_COMMITTED=no`) is not the same fact as global account state being unchanged (`AUTHORITATIVE_POST_ATTEMPT_STATE`, `LOCAL_FAILURE_IMPLIES_GLOBAL_STATE_UNCHANGED=no`). Three cases exhaust how this arises; distinguishing them is not a new owner decision:
    - **No competing commit.** Where no other operation has, by the time this attempt's failure is observed, successfully committed a transition on the same account or credential, authoritative state is still whatever it was before this attempt began — `DISABLED`, credential `VALID` — and this is the only case in which that pair is the correct postcondition (`FAILED_PRECOMMIT_NO_COMPETING_COMMIT_ACCOUNT_STATE=DISABLED`, `FAILED_PRECOMMIT_NO_COMPETING_COMMIT_CREDENTIAL_STATE=VALID`, `FAILED_PRECOMMIT_NO_COMPETING_COMMIT_REPLACEMENT_OBLIGATION=none`).
    - **Concurrent winner already committed.** Where a concurrent attempt on the same single-use credential already reached its own successful commit (the single-commit invariant, above) before this attempt's failure is observed, this losing attempt must not report, restore, or imply `DISABLED`/`VALID` — authoritative state is whatever the winning commit left it as, and this attempt reads and reconciles against that state instead of its own stale snapshot (`CONCURRENT_LOSER_ACCOUNT_STATE=AUTHORITATIVE_CURRENT_STATE`, `CONCURRENT_LOSER_CREDENTIAL_STATE=AUTHORITATIVE_CURRENT_STATE`, `CONCURRENT_LOSER_MAY_RESTORE_DISABLED=no`, `CONCURRENT_LOSER_MAY_RESTORE_CREDENTIAL_VALIDITY=no`, `CONCURRENT_LOSER_CREATES_REPLACEMENT_OBLIGATION=no`, `CONCURRENT_LOSER_IS_NON_AUTHORIZING=yes`). If the credential is observed as consumed, that is because the winning attempt's commit consumed it, not because this losing attempt did (`LOSING_ATTEMPT_SELF_CONSUMES_CREDENTIAL=no`, `OBSERVED_CONSUMED_STATE_MAY_RESULT_FROM_OTHER_COMMITTED_ATTEMPT=yes`).
    - **Already-terminal input.** Where the account is already `CLOSED` when this attempt is made or retried, that is not a fresh pre-commit failure whose postcondition is `DISABLED`/`VALID` — `CLOSED` is terminal (§17.1), and this attempt is a non-authorizing rejection or reconciliation against that terminal state, not a ceremony outcome able to resurrect `DISABLED` or restore recovery authority (`ALREADY_CLOSED_CLOSURE_ATTEMPT_ALLOWED_TO_START_AUTHORIZING_CEREMONY=no`, `ALREADY_CLOSED_RETRY_RESTORES_DISABLED=no`, `ALREADY_CLOSED_RETRY_RESTORES_RECOVERY_CREDENTIAL=no`, `ALREADY_CLOSED_RETRY_ISSUES_REPLACEMENT=no`, `CLOSED_STATE_REMAINS_AUTHORITATIVE=yes`).

    This governs the same ground as the post-commit response-loss rule below and the single-commit invariant above, restated from the losing/retry side rather than the winning one: `FAILED_ATTEMPT_CAN_RESURRECT_TERMINAL_STATE=no`. It selects no database, lock, transaction mechanism, or consensus protocol to provide it — only the invariant a future implementation must satisfy.
  - **A future implementation requirement, stated but not designed here: the account-state transition and the credential consumption must commit as one atomic logical unit.** `CLOSURE_STATE_AND_CREDENTIAL_CONSUMPTION_ATOMIC=yes`. There must be no persisted state in which the transition succeeded without the consumption, or the consumption succeeded without the transition — concretely, for a disabled-account closure, no state in which the account is `CLOSED` but the credential is not consumed, and no state in which the credential is consumed but the account is not `CLOSED` (`STRANDED_DISABLED_CONSUMED_CREDENTIAL_STATE_ALLOWED=no`). This document does not select the database, transaction API, or storage engine that provides that atomicity — only the invariant a future implementation must satisfy.
  - **A lost or delayed response after a successful commit is not a pre-commit failure.** `POST_COMMIT_RESPONSE_FAILURE_DOES_NOT_ROLL_BACK_SECURITY_STATE=yes`; `CLOSURE_RETRY_AFTER_COMMIT_IS_NON_AUTHORIZING=yes`. Authoritative server-side state, not whether the caller received a response, determines outcome: if a timeout, disconnect, or crash prevents the caller from learning that a transition already committed, a subsequent retry or reconciliation attempt must not restore the prior account state, must not re-run the transition, must not issue replacement recovery authority, and must not create a session — it reports or reconciles the already-authoritative state safely instead. This document does not select the idempotency mechanism that provides that; it states only the invariant.
...
| `REACTIVATION_CEREMONY` → `DISABLED` | — | failure, abandonment, expiry, or loss to a competing commit; postcondition governed by authoritative state, not asserted unconditionally — `DISABLED`/credential `VALID` only where no competing commit (this ceremony or a competing closure) has occurred; otherwise whatever the winning commit left, per the ceremony's `Failure` clause below (§11.2) |
...
| `ACCOUNT_CLOSURE_CEREMONY` → `DISABLED` | — | failure, abandonment, expiry, or loss to a competing commit; postcondition governed by authoritative state, not asserted unconditionally — `DISABLED`/credential `VALID` only where no competing commit (this ceremony or a competing reactivation) has occurred; otherwise whatever the winning commit left, per the ceremony's `Failure` clause below (§11.2) |
...
- **After success.** Ceremony authority **ends** at the transition — it is not carried forward, extended, or converted into a session. No prior session is restored; sessions revoked at disable stay revoked, permanently. Any recovery or ceremony material consumed follows the §11.2 lifecycle rules in full, including single-use consumption and invalidation. This ceremony's success is **nonterminal** — the account returns to `ACTIVE` and retains future authentication and recovery capability — so §11.2's **nonterminal** rule applies and replacement issuance is **mandatory**, with continuing recovery capability not treated as available until that issuance is confirmed (`RECOVERY_CREDENTIAL_USED_FOR_NONTERMINAL_RECOVERY=CONSUME_AND_REPLACE_OR_ROTATE`). `FRESH_NORMAL_AUTH_REQUIRED_AFTER_REACTIVATION=yes`: a **fresh ordinary authentication ceremony** (§4.3, §4.4, new bearer credential) is required afterwards, and **only** the ordinary session resulting from it may later attempt a protected private-data request, subject to every normal §7 check. Returning to `ACTIVE` is therefore a precondition for protected access and never itself an access grant.
- **Failure.** Failure, abandonment, expiry, presentation against an already-`CLOSED` account, or a concurrent attempt on the same credential having already committed first — whether that competing commit was this same `DISABLED` → `ACTIVE` reactivation or the **separate** `DISABLED` → `CLOSED` closure ceremony below (§17.1, if D6 permits one) — leaves the account exactly as authoritative state already is, not as this attempt's own snapshot of it. There is no partial success and no fallback path. This attempt's own `DISABLED` → `ACTIVE` transition did not commit, so it consumes nothing itself, restores nothing itself, and creates no replacement obligation of its own; whether the credential it presented is observed as valid or as already consumed depends on authoritative state, not on this attempt's failure alone (`LOCAL_FAILURE_IMPLIES_GLOBAL_STATE_UNCHANGED=no`), distinguished exactly as follows, since none of the three is a new owner decision:
  - **No competing commit.** If no other attempt has committed any transition away from `DISABLED` on this account or credential — neither this reactivation nor a competing closure — authoritative state is unchanged by this failure: the account **remains `DISABLED`** and the presented credential **remains valid** (§11.2).
  - **Competing closure already committed.** If a concurrent closure attempt already committed `DISABLED` → `CLOSED` and consumed the credential first, this losing reactivation attempt must not report or restore `DISABLED`, must not advance the account to `ACTIVE`, and must not report or restore credential validity: authoritative state is `CLOSED` with the credential consumed, and this attempt creates no replacement, no session, and no new authority — it does not undo the closure merely because it lost the race (`CONCURRENT_LOSER_MAY_RESTORE_DISABLED=no`, `CONCURRENT_LOSER_MAY_RESTORE_CREDENTIAL_VALIDITY=no`, `CONCURRENT_LOSER_IS_NON_AUTHORIZING=yes`, `CLOSURE_VS_REACTIVATION_RACE_DEFINED=yes`).
  - **Already-terminal input.** If the account was already `CLOSED` before this attempt began — a stale or replayed presentation, not a race — the same holds: this is a non-authorizing rejection against terminal state, and it must not restore `DISABLED`, advance to `ACTIVE`, or restore recovery authority (`ALREADY_CLOSED_RETRY_RESTORES_DISABLED=no`, `CLOSED_REACTIVATION_ALLOWED=no`, `CLOSED_STATE_REMAINS_AUTHORITATIVE=yes`).

  Under every branch above this attempt must not issue a replacement merely because verification occurred, must not consume the credential merely because the ceremony was entered, and must not create a session (§11.2's commit-point and authoritative-state rules).
...
- **After success.** Ceremony authority **ends** at the transition; it is not carried forward, extended, or converted into a session. The account epoch is incremented, all sessions are revoked permanently, authentication is permanently denied, and `ACCOUNT_CLOSED` is recorded (§12.2, §16.2, §17.4). Any recovery or ceremony material consumed follows the §11.2 lifecycle rules in full, including single-use consumption and invalidation. This ceremony's only success is **terminal**, so §11.2's **terminal** rule applies: the consumed material is permanently invalidated and **no replacement recovery credential is issued**, no outstanding replacement obligation arises or is recorded, no ordinary session is created, and no new authentication or recovery authority of any kind is created (`RECOVERY_CREDENTIAL_USED_FOR_TERMINAL_CLOSURE=CONSUME_WITHOUT_REPLACEMENT`). That follows from `CLOSED` having no continuing authority for a replacement to serve; it is a lifecycle consequence of terminality, not an exception permitting reuse — replay of the consumed credential still fails closed and is still audited — and it carries no private-data or key consequence whatever (§11.2, and the deletion boundary below). If the transition does **not commit**, no consumption occurred at all, and the `Failure` rule immediately below governs instead — not the nonterminal rule, since nothing was consumed for the nonterminal rule to attach an obligation to.
- **Failure.** Failure, abandonment, expiry, presentation against an already-`CLOSED` account, or a concurrent attempt on the same credential having already committed first — whether that competing commit was this same `DISABLED` → `CLOSED` closure or the **separate** `DISABLED` → `ACTIVE` reactivation ceremony above (§17.1, if D5 permits one) — leaves the account exactly as authoritative state already is — not as this attempt's own snapshot of it. There is no partial success, no fallback path, and no partially-closed state. This attempt's own `DISABLED` → `CLOSED` transition did not commit, so it consumes nothing itself, restores nothing itself, and creates no replacement obligation of its own (`FAILED_PRECOMMIT_CLOSURE_CONSUMES_RECOVERY_CREDENTIAL=no`) — resolving `W3_ABORTED_CLOSURE_REPLACEMENT_AUTHORITY_UNDEFINED` without inventing any continuation authority beyond the ceremony's existing bounds. What authoritative state actually reads afterwards depends on whether some other operation already committed (§11.2's commit-point and authoritative-state rules), distinguished exactly as follows, since none of these is a new owner decision:
  - **No competing commit.** If no other attempt has committed any transition away from `DISABLED` on this account or credential — neither this closure nor a competing reactivation — authoritative state is unchanged by this failure: the account **remains `DISABLED`** and any credential presented **remains valid** (`FAILED_CLOSURE_RECOVERY_AUTHORITY_SAFE_STATE_DEFINED=yes`).
  - **Competing closure already committed.** If a concurrent closure attempt already committed `DISABLED` → `CLOSED` and consumed the credential first, this losing attempt must not report or restore `DISABLED`, and must not report or restore credential validity: authoritative state is `CLOSED` with the credential consumed, and this attempt creates no replacement, no session, and no new authority (`CONCURRENT_LOSER_MAY_RESTORE_DISABLED=no`, `CONCURRENT_LOSER_MAY_RESTORE_CREDENTIAL_VALIDITY=no`, `CONCURRENT_LOSER_IS_NON_AUTHORIZING=yes`).
  - **Competing reactivation already committed.** If a concurrent **reactivation** attempt already committed `DISABLED` → `ACTIVE` and consumed the credential first, this losing closure attempt must not restore `DISABLED`, must not report or restore credential validity, must not close the account merely because it lost the race, must not undo the reactivation, and must not create any replacement authority: authoritative state is `ACTIVE` with the credential consumed, and this attempt creates no replacement, no session, and no new authority (`CONCURRENT_LOSER_MAY_RESTORE_DISABLED=no`, `CONCURRENT_LOSER_MAY_RESTORE_CREDENTIAL_VALIDITY=no`, `CONCURRENT_LOSER_IS_NON_AUTHORIZING=yes`, `CLOSURE_VS_REACTIVATION_RACE_DEFINED=yes`). This is not a conversion of one ceremony's outcome into the other — closure and reactivation remain distinct ceremonies, distinct code paths, and are never entered by one another (§17.1's opening invariant); this attempt simply lost a race for the same single-use credential to a different, independently authorized transition.
  - **Already-terminal input.** If the account was already `CLOSED` before this attempt began — a stale or replayed presentation, not a race — the same holds: this is a non-authorizing rejection against terminal state, not a fresh pre-commit failure, and it must not restore `DISABLED` or recovery authority (`ALREADY_CLOSED_RETRY_RESTORES_DISABLED=no`, `ALREADY_CLOSED_RETRY_RESTORES_RECOVERY_CREDENTIAL=no`, `CLOSED_STATE_REMAINS_AUTHORITATIVE=yes`).

  The operation may generate a content-free failure/audit event and remains subject to §15's rate and abuse limits; under every branch above it must not issue a replacement merely because verification occurred, must not consume the credential merely because the ceremony was entered, must not create a session, must not reactivate or resurrect the account, and must not close the account merely because a competing attempt lost (§11.2).
- It is rate-limited at least as strictly as login and recovery (§15), audited content-free (§16), and surfaced to the owner.
...
Neither outcome of D5 determines D6, and neither outcome of D6 determines D5; all four combinations are coherent and defensible. **That logical independence is the entire justification for registering D6 separately**, and it is sufficient on its own. D5 asks whether a `DISABLED` account may use the accepted recovery authority to run a limited **reactivation** ceremony and return to `ACTIVE`. D6 asks whether a `DISABLED` account may use the accepted recovery authority to run a limited **closure** ceremony and reach `CLOSED` **directly, while the account is still `DISABLED` and without first restoring ordinary account authority**. Those are two different outgoing transitions from the same state, entered by two ceremonies that are never the same code path and never convertible into one another (§17.1), and each is decidable with the other held fixed at either of its values.

**What this rationale no longer claims, and why.** Revision 4 argued the separation a second way — that closure is destructive where reactivation is restorative, that permitting D6(b) uniquely hands an attacker a terminal denial-of-access capability, and that the worst case under D5(b) is merely an unwanted reactivation the true owner can observe, audit and disable again. Independent re-QA established that this comparison is **unsupported**, and it is **withdrawn**. If D5 answers (b), whoever completes the reactivation ceremony reaches `ACTIVE`; §17.1 applies the §11.2 lifecycle in full to the consumed recovery material, and — reactivation being a **nonterminal** outcome — that lifecycle mandates issuance of replacement material to whoever completed the ceremony (§11.2); from `ACTIVE`, the ordinary credential-reset ceremony (§11.1, §11.2) is reachable, a fresh ordinary authentication can follow it, and `ACTIVE_ACCOUNT_CLOSURE_AUTHORITY` (§4.6, §17.1, §17.4) then permits closure. Every step is a separate ceremony, and the sequence converts no ceremony authority into session authority and performs no private-data or key operation — it is an inference from the specified lifecycle, not a claim about any existing software, but the specified lifecycle is what the owner is being asked to accept. Therefore:

- `D6_IS_ONLY_PATH_TO_PERMANENT_ACCOUNT_CLOSURE=no`. Under D5(b), a `DISABLED` account can eventually reach `CLOSED` through reactivation followed by the ordinary `ACTIVE` closure path, with D6 answered (a).
- `D6_STRICTLY_GREATER_HARM_THAN_D5=no`. Both D5(b) and D6(b) can culminate in an irreversible closure. No strict ordering of harm between them is asserted here, and none may be inferred from this document.

The owner must therefore weigh **D5(b)'s own compromise consequence as including that composed route** (§26.5), rather than as bounded by an unwanted reactivation. Understating it would be a defect in the decision material even though the state transitions themselves are coherent.

**What D6 decides, stated without any harm comparison.** D6 decides whether a **limited recovery/closure authority may itself directly authorize an irreversible account-lifecycle transition while the account remains `DISABLED`** — in one ceremony, with no intermediate `ACTIVE` state, no replacement-credential issuance, no fresh ordinary authentication, and no interval in which the true owner could observe the account alive again and disable it. That is a distinct authority surface from **restoring ordinary account authority first and requiring the ordinary `ACTIVE` closure ceremony afterwards**, which is what D5(b) with D6(a) offers. The two differ in **which authority is sufficient for the irreversible step, how many independent proofs it costs, and what intermediate states are observable** — not in whether closure is ultimately reachable. Both are legitimate owner answers, and folding the question into D5 would hide it behind a decision the owner might answer for entirely unrelated availability reasons.

**D5/D6 independence matrix.** Conceptual planning only; the table describes what each combination would mean, and **no combination is accepted** (`OWNER_DECISIONS_ACCEPTED=0`).

| D5 | D6 | Outgoing transitions available from `DISABLED` | Can the account reach `CLOSED`, and by what route |
|---|---|---|---|
| **(a)** terminal disable | **(a)** no self-closure | **None.** `DISABLED` has no outgoing transition under Gate 0B and is terminal in practice | **No.** Not through any Gate 0B path; only a future, separately authorized account-management design could change that, and the owner must accept this knowingly (§26.5, §26.6) |
| **(b)** limited self-reactivation | **(a)** no self-closure | Reactivation ceremony only (`DISABLED` → `ACTIVE`) | **Yes, indirectly.** Reactivate to `ACTIVE`, take the §11.2 replacement material, complete a **fresh ordinary authentication**, then close under `ACTIVE_ACCOUNT_CLOSURE_AUTHORITY` with §4.6 re-authentication. Several ceremonies and an ordinary login; the account is observably `ACTIVE` in between |
| **(a)** terminal disable | **(b)** limited self-closure | Closure ceremony only (`DISABLED` → `CLOSED`) | **Yes, directly.** Through the limited `ACCOUNT_CLOSURE_CEREMONY` of §17.1 entered with the accepted recovery authority. Return to `ACTIVE` is not possible at all, so closure is the account's only exit |
| **(b)** limited self-reactivation | **(b)** limited self-closure | Both ceremonies exist and are separate paths | **Yes, by either route**, and the owner of the account chooses which. Direct closure still grants no private-data access, no key authority, no operator authority, and no session (`ACCOUNT_CLOSURE_CEREMONY_PRIVATE_DATA_ACCESS=deny`, §17.1) |

Every cell is coherent, and fixing either decision at either of its values leaves the other genuinely open — which is what logical independence means and is precisely the test that separated D5 from D4. `D5_D6_INDEPENDENCE_MATRIX_PRESENT=yes`; `D5_D6_LOGICAL_INDEPENDENCE=PASS`. Note that the second and third rows both end in a permanently closed account by different sequences, which is exactly why the classification rests on independence rather than on comparative harm.
...
- AUTH_CONTROL: authoritative server-side session state plus account authorization epoch; immediate invalidation; TTL-only models explicitly rejected; no cached ALLOW is authoritative; one governing credential-change revocation rule covering the initiating session (§4.8, F7); in `DISABLED`, every ordinary session is revoked and non-authoritative and `DISABLED_ORDINARY_SESSION_AUTHORITY=invalid`, so no stale session and no ceremony authority can stand in for a live authorization — reactivation ends its own authority at the `DISABLED` → `ACTIVE` transition and a fresh ordinary authentication is required before any protected request (§17.1). The same holds for closure, which revision 3 left unstated: a pre-disable session is **denied** as entry proof to the `ACCOUNT_CLOSURE_CEREMONY` rather than accepted (`DISABLED_CLOSURE_USES_ORDINARY_SESSION=no`), closure-ceremony authority ends at the `DISABLED` → `CLOSED` transition and is never converted into a session, and closure itself increments the epoch so every remaining session is denied on its next protected request (§12.2, §17.1, §17.4). Revision 6 adds the credential-lifecycle half of the same invariant: recovery material consumed by a **successful terminal** closure is invalidated and **not replaced** (`RECOVERY_CREDENTIAL_USED_FOR_TERMINAL_CLOSURE=CONSUME_WITHOUT_REPLACEMENT`, §11.2), so no stale recovery authority survives the transition to be presented later, while a **nonterminal** reactivation must issue a confirmed replacement rather than leave the account without recovery capability — under both outcomes a consumed credential is unreplayable. Revision 7 completes the credential half of this invariant for the non-committing case: consumption is defined to occur only at successful commit (`RECOVERY_CREDENTIAL_CONSUMPTION_COMMIT_POINT=SUCCESSFUL_COMMIT_OF_THE_AUTHORIZED_RECOVERY_OR_ACCOUNT_STATE_TRANSITION`, §11.2), so a ceremony that fails, is abandoned, or expires before its transition commits consumes nothing and leaves any presented credential valid (`FAILED_PRECOMMIT_CLOSURE_CONSUMES_RECOVERY_CREDENTIAL=no`) rather than stranding a `DISABLED` account with a consumed credential and no authority to act on it; and a response lost after a successful commit authorizes no retry to roll back state, re-run the transition, or mint fresh authority (`CLOSURE_RETRY_AFTER_COMMIT_IS_NON_AUTHORIZING=yes`). Revision 8 conditions that non-committing-case rule on authoritative state rather than stating it unconditionally: a losing concurrent attempt must not report or restore `DISABLED`/valid once a competing commit has already changed authoritative state, and a retry against an account already authoritatively `CLOSED` is a non-authorizing reconciliation against that terminal state, never a route back to a stale `DISABLED`/valid reading (`FAILURE_POSTCONDITION_IS_DERIVED_FROM_AUTHORITATIVE_CURRENT_STATE=yes`, §11.2, §17.1). Revision 9 closes the remaining gap independent re-QA found in that conditioning: the "competing commit" §17.1 checked for was, on the closure side, only a competing closure — a competing **reactivation** committing first was not named, so a losing closure attempt could still be read as restoring `DISABLED`/valid against an authoritative `ACTIVE`/consumed state. Both `Failure` clauses, and the transition-table rows summarizing them, now name **every** competing transition away from `DISABLED` — closure and reactivation alike — not only the same-ceremony case (§17.1).
...
- FUTURE_TEST: the §12.3 invariant — after accepted revocation, the next protected request is denied — exercised for logout, revoke-one, revoke-all, credential reset (including the initiating session), recovery, disable, and closure **from both originating states**; the disabled-account tests distinguishing ordinary denial from the limited reactivation and closure ceremonies, and asserting that a successful `DISABLED` → `ACTIVE` transition still denies protected access until a fresh ordinary authentication succeeds (§22 tests 11, 35, 37); the disabled-closure entry-authority and containment tests (§22 tests 40-43); the outcome-aware recovery-credential consumption assertions (§22 tests 12, 37, 44); plus the CASE A/CASE B qualification tests (§22 tests 22, 23); and the commit-point and authoritative-state tests for pre-commit closure failure, post-commit response loss, concurrent-loser reconciliation, and already-closed retry (§22 tests 45-48).

**THREAT-021 — privilege escalation**
- AUTH_CONTROL: single human role; no admin principal to escalate to; no operator account-recovery, credential-reset, reactivation or **closure** power (`OPERATOR_ACCOUNT_CLOSURE_POWER=none`, `APPLICATION_ADMIN_CLOSURE_POWER=none`, §9.4); fail-closed on ambiguous role or owner state; service principals narrowly scoped **and workspace-bound**, with explicit, revocable, expiring grants and no interactive authentication path (§3.2, §8.3); the §17.1 reactivation ceremony bounded to status transition only, with no private-data authority; the §17.1 `ACCOUNT_CLOSURE_CEREMONY` bounded the same way and no more permissively — `ACCOUNT_CLOSURE_CEREMONY_PRIVATE_DATA_ACCESS=deny`, no read, write, search, export, workspace access, ownership transfer, key unwrap, key destruction or data-recovery authority, and no conversion of ceremony authority into a session; and, where that ceremony succeeds terminally, no replacement recovery credential and no new authentication or recovery authority is created by it (`RECOVERY_CREDENTIAL_USED_FOR_TERMINAL_CLOSURE=CONSUME_WITHOUT_REPLACEMENT`, §11.2), so a closure cannot mint the authority a closed account is not entitled to hold. Revision 7 adds the pre-commit half: a closure or reactivation ceremony that fails, is abandoned, or expires before its transition commits mints no authority of any kind, consumed or otherwise — it does not issue a replacement, does not open a session, and does not reactivate the account merely because the ceremony was entered or the credential verified (§11.2). Revision 8 closes the remaining gap in that pre-commit half: a losing concurrent attempt or an already-`CLOSED` retry mints no authority either, and specifically must not restore `DISABLED` or credential validity merely because it is the attempt that failed to commit (`CONCURRENT_LOSER_IS_NON_AUTHORIZING=yes`, `ALREADY_CLOSED_CLOSURE_ATTEMPT_ALLOWED_TO_START_AUTHORIZING_CEREMONY=no`, §11.2, §17.1). Revision 9 extends the same non-authorization to the specific case a losing attempt now must not use its loss to *act*: a closure attempt that loses a race to a competing reactivation must not close the account, must not undo the reactivation, and must not treat losing as license to fall back to some other transition — it mints nothing at all (§17.1). Revision 9 also corrects the concurrency test oracle itself: `AT_MOST_ONE_SUCCESSFUL_COMMIT` (never `EXACTLY_ONE`) now governs both the escalation and the stale-authorization threat rows, and test 47 asserts the zero-commit case explicitly rather than only the one-winner case (§22 tests 12, 47).
- DESIGN_SECTION: §1.3, §3.2, §8.2, §8.3, §9.4, §11.2, §17.1, §17.3.
...
12. **Recovery abuse and lifecycle** — recovery is rate-limited, progressively delayed, single-use, and revokes all sessions on completion; at most one recovery credential is valid at a time; regeneration requires re-authentication and invalidates the previous credential on confirmed issuance of its replacement; a failed or unconfirmed replacement leaves a defined state rather than an ambiguous one; two or more concurrent consumption attempts yield **at most one** success — never more than one, and possibly zero if every attempt fails or is abandoned before any of them commits (`CONCURRENCY_INVARIANT=AT_MOST_ONE_SUCCESSFUL_COMMIT`, not "exactly one"; §22 test 47); and the consumption rule is **outcome-aware** — a consumption whose ceremony leaves the account usable mandates a confirmed replacement and records an outstanding obligation if issuance fails, while a consumption whose ceremony terminally closes the account issues no replacement and records no outstanding obligation — with the consumed credential permanently unreplayable in **both** cases (`RECOVERY_LIFECYCLE_OUTCOME_AWARE=yes`, §11.2).
...
37. **Reactivation success is bounded (D5)** — the ceremony's only success transition is `DISABLED` → `ACTIVE` (`REACTIVATION_SUCCESS_TRANSITION=DISABLED_TO_ACTIVE_ONLY`); closure is never reported or recorded as a reactivation success; ceremony authority **ends** at the transition and cannot be reused, extended or converted into a session; any recovery material consumed follows the §11.2 lifecycle including single-use invalidation and, this transition being **nonterminal**, including mandatory replacement issuance — assert the predecessor cannot be replayed, that a confirmed replacement is issued, and that continuing recovery capability is not reported as available until that issuance is confirmed (`RECOVERY_CREDENTIAL_USED_FOR_NONTERMINAL_RECOVERY=CONSUME_AND_REPLACE_OR_ROTATE`, §11.2); and a successfully reactivated `ACTIVE` account **still** cannot reach private data until a fresh ordinary login succeeds and every normal §7 check passes (§17.1).
...
44. **Closure is terminal, and closure is not deletion** — assert that a `CLOSED` account cannot reactivate (`CLOSED_REACTIVATION_ALLOWED=no`, test 36), cannot be presented to the closure ceremony again, and cannot use any closure or reactivation path as an access-recovery mechanism (`CLOSED_STATE_TERMINAL_FOR_GATE_0B=yes`); that the account-status transition occurs **without** any private-data deletion, key destruction, cryptographic erasure, backup eradication or workspace destruction being performed, claimed, or recorded in any audit event or user-facing message (`ACCOUNT_CLOSURE_IMPLIES_PRIVATE_DATA_DELETION=no`, `ACCOUNT_CLOSURE_IMPLIES_KEY_DESTRUCTION=no`); and that an implementation **fails qualification** if any closure path asserts data or key deletion, or invokes a key-unwrap, key-destruction or data-recovery mechanism, that a later gate has not separately accepted (`PRIVATE_DATA_DISPOSITION_AFTER_ACCOUNT_CLOSURE=UNRESOLVED_PENDING_LATER_GATES`; §17.1, §20). Assert additionally that a **successful** terminal closure — its `DISABLED` → `CLOSED` transition having actually **committed** — consumes the recovery material presented to the ceremony **without** issuing any replacement recovery credential, records and surfaces no outstanding replacement obligation, creates no ordinary session, and creates no new authentication or recovery authority of any kind, while the consumed credential remains permanently unreplayable (test 29) — and that a closure attempt whose transition does **not commit** has consumed nothing at all: absent any other commit having already consumed it, the presented credential remains valid, the account remains `DISABLED`, and no replacement obligation is created by that attempt; where another attempt already committed the closure first, this attempt must instead report the account `CLOSED` and the credential consumed, and must not restore `DISABLED` or credential validity (test 47) (`RECOVERY_CREDENTIAL_USED_FOR_TERMINAL_CLOSURE=CONSUME_WITHOUT_REPLACEMENT`, `RECOVERY_CREDENTIAL_REPLAY_AFTER_USE_ALLOWED=no`, `RECOVERY_CREDENTIAL_CONSUMPTION_COMMIT_POINT=SUCCESSFUL_COMMIT_OF_THE_AUTHORIZED_RECOVERY_OR_ACCOUNT_STATE_TRANSITION`, §11.2).
45. **Pre-commit closure or reactivation failure leaves recovery authority intact, absent a competing commit** — if D6 (or D5) permits the ceremony: for an attempt that verifies the presented recovery credential but is then abandoned, times out, expires, or fails a later validation step before its transition commits, **and no other concurrent or prior attempt on the same account or credential has already committed**, assert the account remains in its originating state (`DISABLED`), the presented credential **remains valid and unconsumed**, no replacement recovery credential or obligation is created by that attempt, no ordinary session or private-data authority is created, and the account is not reactivated or closed (`FAILED_PRECOMMIT_CLOSURE_CONSUMES_RECOVERY_CREDENTIAL=no`, `FAILED_CLOSURE_RECOVERY_AUTHORITY_SAFE_STATE_DEFINED=yes`, §11.2, §17.1). This test's precondition — no competing commit — is what distinguishes it from test 47's concurrent-loser case and test 48's already-closed case; the three exercise different traces and do not share an expected outcome. Applied simultaneously to **each** of two or more concurrent attempts on the same account or credential where **none** of them commits, this test's precondition ("no other attempt has committed") holds for every one of them at once — this is the same both/all-fail-before-commit trace that test 47's zero-commit case names explicitly, and this test's outcome (`DISABLED`/`VALID`, no consumption, no replacement obligation, no winner to reconcile against) is what governs it.
46. **Post-commit response loss is not a rollback trigger** — for a `DISABLED` → `CLOSED` transition (or a `DISABLED` → `ACTIVE` reactivation) that commits successfully but whose success response is lost to the caller (timeout, disconnect, or crash), assert that authoritative state reads as the committed state with the credential `CONSUMED`, and that a subsequent retry or reconciliation attempt using the same credential does not restore the prior state, does not re-run the transition, does not issue replacement recovery authority, and does not create a session (`POST_COMMIT_RESPONSE_FAILURE_DOES_NOT_ROLL_BACK_SECURITY_STATE=yes`, `CLOSURE_RETRY_AFTER_COMMIT_IS_NON_AUTHORIZING=yes`, §11.2).
47. **Concurrent commit attempts on one credential** — for two or more concurrent attempts presenting the same single-use recovery credential toward a committing transition — whether the same ceremony racing itself (e.g. two closures) or two **different** ceremonies racing each other for the same credential (e.g. a closure against a reactivation, §17.1) — the governing invariant is **at most one successful commit, never exactly one** (`CONCURRENCY_INVARIANT=AT_MOST_ONE_SUCCESSFUL_COMMIT`, `RECOVERY_CREDENTIAL_SINGLE_COMMIT_SEMANTICS=yes`, `CONCURRENT_SUCCESSFUL_REPLAY_ALLOWED=no`, §11.2), and this holds even when more than one attempt passes verification before either commits. This test asserts both of the invariant's exhaustive cases, not only the first:
    - **Exactly one attempt commits.** Assert every other concurrent or subsequently replayed attempt is denied as an invalid credential, and that the losing attempt(s) — regardless of which ceremony they were, and regardless of which ceremony won — report or reconcile against the winner's committed authoritative state rather than their own pre-race snapshot: a losing attempt must not restore `DISABLED`, must not restore credential validity, must not undo or convert the winning transition, and must not create a replacement obligation, a session, or any new authority, regardless of how far its own verification proceeded (`CONCURRENT_LOSER_MAY_RESTORE_DISABLED=no`, `CONCURRENT_LOSER_MAY_RESTORE_CREDENTIAL_VALIDITY=no`, `CONCURRENT_LOSER_CREATES_REPLACEMENT_OBLIGATION=no`, `CONCURRENT_LOSER_IS_NON_AUTHORIZING=yes`, `CLOSURE_VS_REACTIVATION_RACE_DEFINED=yes`).
    - **Zero attempts commit.** If every concurrent attempt is abandoned, times out, fails validation, or otherwise fails before any of them commits, assert there is **no winner to reconcile against**: authoritative state remains whatever it was before any attempt began, no replacement obligation is created by any attempt, and no session or new authority is created by any attempt (`BOTH_ATTEMPTS_FAIL_BEFORE_COMMIT_DEFINED=yes`, `BOTH_FAIL_REQUIRES_WINNER=no`; governed by test 45's outcome applied to each concurrent attempt). This is a distinct, required trace, not a narrower reading of the one-commit case above — a test suite that exercises only the one-winner case establishes `EXACTLY_ONE_SUCCESSFUL_COMMIT`, which is **not** what this document requires; it must also exercise the zero-commit trace to establish `AT_MOST_ONE_SUCCESSFUL_COMMIT`.

48. **Retry against an already-`CLOSED` account is non-authorizing, not a fresh pre-commit failure** — for a closure or reactivation attempt presented against an account already authoritatively `CLOSED` (including a stale, replayed, or post-response-loss retry), assert that the account remains `CLOSED`, that any presented credential is evaluated against its actual authoritative validity rather than reported valid by default, that the attempt does not begin an authorizing ceremony capable of changing account state, does not restore `DISABLED`, does not restore recovery authority, and does not issue a replacement credential (`ALREADY_CLOSED_CLOSURE_ATTEMPT_ALLOWED_TO_START_AUTHORIZING_CEREMONY=no`, `ALREADY_CLOSED_RETRY_RESTORES_DISABLED=no`, `ALREADY_CLOSED_RETRY_RESTORES_RECOVERY_CREDENTIAL=no`, `ALREADY_CLOSED_RETRY_ISSUES_REPLACEMENT=no`, `CLOSED_STATE_REMAINS_AUTHORITATIVE=yes`, §11.2, §17.1).

`FUTURE_AUTH_VALIDATION_PLAN_COMPLETE=yes` — in the sense that every area repaired in this and the previous revisions now has a named future test with an expected outcome, including the R1 owner-decision gate (test 39), the R2 reactivation state machine (tests 11, 33, 35, 36, 37), the R3 Gate 0C boundary (tests 34, 38) and the S1 closure authority (tests 8, 11, 32, 40, 41, 42, 43, 44). Tests 40-44 were added by revision 4 and are the only additions; no existing test was rewritten except tests 8, 11, 32 and 39, each of which previously spoke of "closure" or "`D1`-`D5`" in terms revision 4 made more precise. Revision 6 added **no** test and extended exactly three — tests 12, 37 and 44 — so that the outcome-aware §11.2 consumption rule is asserted on both sides: mandatory confirmed replacement after a nonterminal reactivation (test 37), no replacement and no new authority after a terminal closure (test 44), and the general rule with replay rejection under both outcomes (test 12). Revision 7 extended test 44 to state the commit-point condition explicitly and added exactly three new tests — 45, 46 and 47 — covering pre-commit ceremony failure, post-commit response loss, and concurrent single-commit/replay guarantees, resolving `W3_ABORTED_CLOSURE_REPLACEMENT_AUTHORITY_UNDEFINED`. Revision 8 narrowed test 44's non-commit clause and test 45's precondition to require no competing commit, extended test 47 with the losing attempt's postcondition, and added exactly one new test — 48 — covering retry against an already-`CLOSED` account, resolving `W4_NONCOMMIT_FAILURE_POSTCONDITIONS_CONFLICT_WITH_COMMITTED_STATE` by making every failure-postcondition test conditional on authoritative state instead of leaving tests 44/45 and 46/47 asserting incompatible outcomes on the same concurrent trace. Revision 9 added **no** new test and revised exactly two — 12 and 47 — plus one clarifying addition to test 45's cross-reference, resolving the independent re-QA's remaining findings without reopening tests 44, 46 or 48: test 12's "exactly one success" was replaced with `AT_MOST_ONE_SUCCESSFUL_COMMIT`, matching the invariant name already used elsewhere and never contradicted by it; test 47 was split into its two exhaustive cases — exactly one commit, and zero commits — because its prior wording unconditionally required a losing attempt to reconcile against a winner's state even when no winner existed, which test 45 already correctly handled for a single non-committing attempt but test 47 did not extend to the multi-attempt zero-commit trace; and test 45 gained one sentence making explicit that its precondition, applied to each of several concurrent non-committing attempts, is exactly that zero-commit trace. This resolves `W5_CLOSURE_REACTIVATION_RACE_AND_ZERO_COMMIT_ORACLE_GAPS` (§17.1, §22). Not in the sense that any test exists: none of these is implemented, and none may be implemented before the implementation gate is separately authorized.
...
SECURITY_TRADEOFFS= What distinguishes this decision from D5 is **which authority is sufficient for an irreversible step, and how directly** — **not** a claim that D6 is more harmful than D5. Under (b), the accepted recovery authority becomes sufficient **by itself, in a single ceremony**, to end account access permanently: the account never passes through `ACTIVE`, no replacement credential is issued, no fresh ordinary authentication is demanded, no §4.6 re-authentication intervenes, and there is no interval in which the true owner could observe the account alive again and disable it. An unwanted closure cannot be undone under Gate 0B (`CLOSED_REACTIVATION_ALLOWED=no`, `CLOSED_STATE_TERMINAL_FOR_GATE_0B=yes`). **This is explicitly not an assertion that (b) carries strictly greater harm than D5(b), nor that (b) is the only route to a permanently closed account.** If D5 answers (b), whoever completes the reactivation ceremony reaches `ACTIVE`, takes the §11.2 replacement material, can reach the credential-reset ceremony, can authenticate freshly, and can then close under `ACTIVE_ACCOUNT_CLOSURE_AUTHORITY` — an irreversible closure by a longer sequence (§17.3, §26.5). `D6_IS_ONLY_PATH_TO_PERMANENT_ACCOUNT_CLOSURE=no`; `D6_STRICTLY_GREATER_HARM_THAN_D5=no`. What (b) changes is **directness and the number of independent proofs required**, which is why under (b) the §11.2 lifecycle requirements and §15 rate limits become load-bearing against an availability attack at the very first ceremony rather than across a sequence of them. (b) also adds a second ceremony to specify, bound, rate-limit, audit and test, and a second surface that must not be allowed to grow into an operator on-ramp or acquire private-data authority at implementation time. (a) adds no ceremony, no new authority and no new surface, and cannot be abused because it does not exist; its cost is real but narrower than it first appears — a disabled owner cannot end their account relationship on their own, and under D5(a) as well they are left in a state with no exit, which for case material is a user-facing consequence the owner should choose deliberately rather than inherit. Under **both** options the §17.1 invariants hold unchanged: `DISABLED_CLOSURE_USES_ORDINARY_SESSION=no`; no operator, support function or admin principal may close an account (`OPERATOR_ACCOUNT_CLOSURE_POWER=none`, `APPLICATION_ADMIN_CLOSURE_POWER=none`); `CLOSED` is terminal and never an access-recovery route; and closure never implies private-data deletion or key destruction. Neither option creates an operator backdoor, and neither is permitted to acquire one at implementation time. Neither option decides what happens to stored private content: `PRIVATE_DATA_DISPOSITION_AFTER_ACCOUNT_CLOSURE=UNRESOLVED_PENDING_LATER_GATES`.

RECOMMENDATION= (a) for the first private-data deployment, on the same reasoning that recommends D5(a) and with one addition specific to closure: (b) would make a single limited ceremony sufficient to reach an irreversible, unrecoverable outcome **directly**, and (a) can be revisited later without having already shipped such a path. That is a statement about the directness and reversibility of one path — **not** a claim that D6(b) is more harmful than D5(b), and **not** a claim that D6 is the only route to a closed account; both are withdrawn in §17.3, and D5(b) carries its own composed route to irreversible closure that the owner must weigh under §26.5. If the owner does want a disabled owner to be able to end the relationship without first reactivating, (b) is legitimate and is fully specified in §17.1 so it can be chosen without redesign; it must then be chosen explicitly, and it should be paired with the §15 limits and the §11.2 single-use lifecycle treated as security controls rather than hygiene. **This recommendation is not an acceptance** (`OWNER_DECISIONS_ACCEPTED=0`), and it is **not** an implementation default: if D6 is unanswered, implementation does not begin (§23).
...
D5_D6_LOGICAL_INDEPENDENCE=PASS

D6_IS_ONLY_PATH_TO_PERMANENT_ACCOUNT_CLOSURE=no

D6_STRICTLY_GREATER_HARM_THAN_D5=no
...
RECOVERY_CREDENTIAL_CONSUMPTION_COMMIT_POINT=SUCCESSFUL_COMMIT_OF_THE_AUTHORIZED_RECOVERY_OR_ACCOUNT_STATE_TRANSITION

RECOVERY_CREDENTIAL_VERIFICATION_EQUALS_CONSUMPTION=no

CEREMONY_ENTRY_EQUALS_CONSUMPTION=no

SUCCESSFUL_COMMIT_CONSUMES_CREDENTIAL=yes

CLOSURE_STATE_AND_CREDENTIAL_CONSUMPTION_ATOMIC=yes

STRANDED_DISABLED_CONSUMED_CREDENTIAL_STATE_ALLOWED=no

FAILED_CLOSURE_RECOVERY_AUTHORITY_SAFE_STATE_DEFINED=yes

FAILED_PRECOMMIT_CLOSURE_CONSUMES_RECOVERY_CREDENTIAL=no

NONCOMMITTING_ATTEMPT_MAY_NOT_CHANGE_OR_ROLL_BACK_AUTHORITATIVE_STATE=yes

FAILURE_POSTCONDITION_IS_DERIVED_FROM_AUTHORITATIVE_CURRENT_STATE=yes

LOCAL_FAILURE_IMPLIES_GLOBAL_STATE_UNCHANGED=no

CONCURRENT_LOSER_IS_NON_AUTHORIZING=yes

CLOSED_STATE_REMAINS_AUTHORITATIVE=yes

FAILED_ATTEMPT_CAN_RESURRECT_TERMINAL_STATE=no

POST_COMMIT_RESPONSE_FAILURE_DOES_NOT_ROLL_BACK_SECURITY_STATE=yes

CLOSURE_RETRY_AFTER_COMMIT_IS_NON_AUTHORIZING=yes

RECOVERY_CREDENTIAL_SINGLE_COMMIT_SEMANTICS=yes

CONCURRENT_SUCCESSFUL_REPLAY_ALLOWED=no
...
The next step is independent QA of the exact commit introducing this revision, followed by operator review of the §26 register's six decisions (D1-D6), and then a separate owner authorization decision about whether any later gate begins. This revision repairs `W4_NONCOMMIT_FAILURE_POSTCONDITIONS_CONFLICT_WITH_COMMITTED_STATE`, the one remaining material finding independent review raised against revision 7; it does not accept the design.
