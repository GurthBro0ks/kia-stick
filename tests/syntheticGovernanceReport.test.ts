...
describe("v0.7.18 synthetic governance bundle report", () => {
  ...
  it("tracks package, feature, and queue state without product or prompt drift", () => {
    ...
    expect(featureList.v0718_synthetic_governance_bundle_report.status).toBe("ready_for_operator_bundle_review");
    expect(featureList.v0718_synthetic_governance_bundle_report.queue_015_status).toBe("blocked");
    expect(featureList.v0718_synthetic_governance_bundle_report.queue_033_status).toBe("needs_review");
    ...
    expect(q033?.status).toBe("needs_review");
