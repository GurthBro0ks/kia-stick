...
const phase = "KIA-Stick-v1.1.17-next-safe-work-checkpoint";
const docPath = "docs/v1.1.17-next-safe-work-checkpoint.md";

describe("v1.1.17 next safe work checkpoint", () => {
  it("records the local implementation checkpoint without approving push or blocked work", () => {
    const doc = readFileSync(docPath, "utf8");
    for (const required of [phase, "KIA-Stick-v1.1.13 records accepted pushed v1.1.12 closeout at `b911fd1`.", "KIA-Stick-v1.1.14 refreshes the static current accepted pushed state contract.", "KIA-Stick-v1.1.15 refreshes Settings -> Operator Status current accepted pushed copy from the contract.", "KIA-Stick-v1.1.16 verifies proof-index and closeout-helper post-push freshness guards.", "This local checkpoint does not push.", "Manual QA for this local checkpoint is `pending_operator_review` until the operator records OPERATOR_QA_PASS for this proof.", "Operator note: after every future closeout push, update `data/current-accepted-pushed-state.json` in a separate fake-only checkpoint before baseline-sensitive UI or tooling work.", "Next/PostCSS remains parked as `WARN_SAFE_NEXT_TARGET_UNCLEAR`.", "`queue-015-v07-first-real-doc-gate-request` remains blocked.", "This checkpoint does not approve package mutation, service changes, notifications, push, or real-doc capability."]) expect(doc).toContain(required);
  });

  it("tracks the checkpoint without package, queue, or real-doc drift", () => {
    const state = JSON.parse(readFileSync("feature_list.json", "utf8")).v1117_next_safe_work_checkpoint;
    expect(state.phase).toBe(phase);
    expect(state.status).toBe("accepted");
    expect(state.current_accepted_pushed_short_commit).toBe("b911fd1");
