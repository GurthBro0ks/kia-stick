...
const phase = "KIA-Stick-v1.1.12-next-safe-work-checkpoint";
const docPath = "docs/v1.1.12-next-safe-work-checkpoint.md";

describe("v1.1.12 next safe work checkpoint", () => {
  ...
  it("tracks the checkpoint without package, queue, or real-doc drift", () => {
    ...
    expect(state.current_accepted_pushed_short_commit).toBe("628fbd4");
    expect(state.local_checkpoint_validation).toBe("PASS");
    expect(state.local_checkpoint_pushed).toBe(false);
    expect(state.local_checkpoint_manual_qa_status).toBe(
