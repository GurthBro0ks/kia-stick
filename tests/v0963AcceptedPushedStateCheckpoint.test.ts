...
const phase = "KIA-Stick-v0.9.63-accepted-pushed-state-checkpoint";
const docPath = "docs/v0.9.63-accepted-pushed-state-checkpoint.md";

describe("v0.9.63 accepted pushed state checkpoint", () => {
  it("records the v0.9.62 accepted pushed baseline without reopening blocked gates", () => {
  ...
  it("tracks the accepted pushed checkpoint in feature state", () => {
    ...
    const state = featureList.v0963_accepted_pushed_state_checkpoint;

    expect(state.phase).toBe(phase);
    expect(state.status).toBe("accepted_pushed_recorded");
    expect(state.accepted_pushed_commit).toBe("d1a31cdb0dea70a09d62a27e59351b8ab356dfad");
    expect(state.accepted_pushed_short_commit).toBe("d1a31cd");
