...
describe("public Settings summary and operator diagnostics split", () => {
  it("shows a concise accepted capability, build, source, privacy, and safety summary by default", () => {
    ...
    expect(html).toContain("Public CBA Annual-Leave Cited Grievance Outline and Automatic Routing Repair");
    expect(html).toContain("9a66d37");
    ...
    expect(html).toContain("KIA-Stick-public-steward-workflow-platform-bundle-1-discipline-copy-repair");
    expect(html).toContain("validation PASS; pushed no; manual QA PASS");
    expect(html).toContain("closeout pending explicit authorization");
  ...
  it("renders the complete existing governance ledger only when diagnostics are opened", () => {
    ...
    expect(html).toContain("repository equality verified at 370979a");
    expect(html).toContain("accepted feature checkpoint is distinct from the repository/closeout recording point");
    expect(html).toContain("Accepted-state recording baseline</dt><dd>370979a7108a4876fea13161d928fedc558f2fed");
    expect(html).toContain("Latest pushed repository closeout commit</dt><dd>370979a7108a4876fea13161d928fedc558f2fed");
    ...
    expect(html).not.toContain("HEAD == origin/main at 9a66d37");
    expect(html).not.toContain("HEAD equal to origin/main at 9a66d37");
