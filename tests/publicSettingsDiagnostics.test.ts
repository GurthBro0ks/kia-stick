...
describe("public Settings summary and operator diagnostics split", () => {
  it("shows a concise accepted capability, build, source, privacy, and safety summary by default", () => {
    ...
    expect(html).toContain("Public Steward Workflow Platform Bundle 3 Core Repairs and Runtime Truth/Favicon Fix");
    expect(html).toContain("e8a4499");
    ...
    expect(html).toContain("Bundle 3 Core Repair Post-Push Accepted-State Refresh");
  ...
  it("renders the complete existing governance ledger only when diagnostics are opened", () => {
    ...
    expect(html).toContain("repository equality verified at 5eb113a");
    expect(html).toContain("accepted feature checkpoint is distinct from the repository/closeout recording point");
    expect(html).toContain("Accepted-state recording baseline</dt><dd>5eb113ae03d6b3db52e840ea2d1b9b5212b6e91f");
    expect(html).toContain("Latest pushed repository closeout commit</dt><dd>5eb113ae03d6b3db52e840ea2d1b9b5212b6e91f");
    ...
    expect(html).toContain("Current working bundle</dt><dd>Bundle 3 Core Repair Post-Push Accepted-State Refresh; validation PASS; pushed no; manual QA pending operator review");
    ...
    expect(html).not.toContain("HEAD == origin/main at e8a4499");
    expect(html).not.toContain("HEAD equal to origin/main at e8a4499");
