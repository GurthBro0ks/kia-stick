...
describe("public Settings summary and operator diagnostics split", () => {
  it("shows a concise accepted capability, build, source, privacy, and safety summary by default", () => {
    ...
    expect(html).toContain("Public Steward Workflow Platform Bundle 1 and Discipline Copy Repair");
    expect(html).toContain("ea0ce8d");
    ...
    expect(html).toContain("KIA-Stick-public-steward-workflow-platform-bundle-1-post-push-accepted-state-refresh");
    expect(html).toContain("validation PASS; pushed no; manual QA PASS; closeout pending explicit authorization");
  ...
  it("renders the complete existing governance ledger only when diagnostics are opened", () => {
    ...
    expect(html).toContain("repository equality verified at 24abecd");
    expect(html).toContain("accepted feature checkpoint is distinct from the repository/closeout recording point");
    expect(html).toContain("Accepted-state recording baseline</dt><dd>24abecd2ba6a7018e5eb58b113e99ad90196eb07");
    expect(html).toContain("Latest pushed repository closeout commit</dt><dd>24abecd2ba6a7018e5eb58b113e99ad90196eb07");
    ...
    expect(html).not.toContain("HEAD == origin/main at ea0ce8d");
    expect(html).not.toContain("HEAD equal to origin/main at ea0ce8d");
