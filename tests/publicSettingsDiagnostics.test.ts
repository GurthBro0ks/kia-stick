...
describe("public Settings summary and operator diagnostics split", () => {
  it("shows a concise accepted capability, build, source, privacy, and safety summary by default", () => {
    ...
    expect(html).toContain("Public Steward Workflow Platform Bundle 2 and Employee Claims Copy Repair");
    expect(html).toContain("3baedc9");
    ...
    expect(html).toContain("Public Steward Workflow Platform Bundle 2");
  ...
  it("renders the complete existing governance ledger only when diagnostics are opened", () => {
    ...
    expect(html).toContain("repository equality verified at a79ce76");
    expect(html).toContain("accepted feature checkpoint is distinct from the repository/closeout recording point");
    expect(html).toContain("Accepted-state recording baseline</dt><dd>a79ce762b6bcabb87b928dd03d3748f187b2ea86");
    expect(html).toContain("Latest pushed repository closeout commit</dt><dd>a79ce762b6bcabb87b928dd03d3748f187b2ea86");
    ...
    expect(html).toContain("Current working bundle</dt><dd>Public Steward Workflow Platform Bundle 2; validation PASS; pushed no; manual QA pending operator review");
    expect(html).toContain("pending operator review");
    ...
    expect(html).not.toContain("HEAD == origin/main at 3baedc9");
    expect(html).not.toContain("HEAD equal to origin/main at 3baedc9");
