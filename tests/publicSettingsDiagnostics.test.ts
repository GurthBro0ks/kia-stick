...
describe("public Settings summary and operator diagnostics split", () => {
  ...
  it("renders the complete existing governance ledger only when diagnostics are opened", () => {
    ...
    expect(html).toContain("repository equality verified at aabfaeb");
    expect(html).toContain("accepted feature checkpoint is distinct from the repository/closeout recording point");
    expect(html).toContain("Accepted-state recording baseline</dt><dd>aabfaebe3a50bc54cd89ffefaffa69fab734099a");
    expect(html).toContain("Latest pushed repository closeout commit</dt><dd>aabfaebe3a50bc54cd89ffefaffa69fab734099a");
