...
const phase = "KIA-Stick-v0.9.87-next-large-work-checkpoint";
const docPath = "docs/v0.9.87-next-large-work-checkpoint.md";

describe("v0.9.87 next large-work checkpoint", () => {
  it("records the local implementation checkpoint without approving push or blocked work", () => {
    ...
    for (const required of [
      ...
      "v0.9.84 clarifies proof index wording",
      "v0.9.85 refreshes closeout helper proof-chain selection",
      "v0.9.86 refreshes operator summary copy",
      ...
      "package mutation, service changes, notifications, or push.",
    ]) {
  ...
  it("tracks the checkpoint without package, queue, or real-doc drift", () => {
    ...
    const state = featureList.v0987_next_large_work_checkpoint;

    expect(state.phase).toBe(phase);
    expect(state.status).toBe("accepted");
    expect(state.current_accepted_pushed_short_commit).toBe("bc8fbef");
    expect(state.local_checkpoint_validation).toBe("PASS");
    expect(state.local_checkpoint_pushed).toBe(false);
    expect(state.local_checkpoint_manual_qa_status).toBe("PENDING");
