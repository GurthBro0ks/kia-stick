...
describe("v1.1.47 next safe work checkpoint", () => {
  it("keeps the refresh local-only and blocked states intact", () => {
    const state = JSON.parse(readFileSync("feature_list.json", "utf8")).v1147_next_safe_work_checkpoint;
    expect(state.current_accepted_pushed_short_commit).toBe("0286f03");
