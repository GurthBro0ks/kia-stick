...
describe("accepted-state governance loop break", () => {
  it("records the pushed Bundle 3 core repairs as current while retaining the prior Bundle 3 capability as history", () => {
    ...
    expect(currentAcceptedPushedState.local_implementation_proof_dir).toBe("/home/mint/kia-stick-local-proofs/proof_kia_stick_accessibility_saved_state_qa_failure_repair_local_commit_20260813T133538Z");
    expect(currentAcceptedPushedState.local_bundle_operator_qa_pass_proof_dir).toBe("/home/mint/kia-stick-local-proofs/proof_kia_stick_accessibility_saved_state_qa_failure_repair_local_commit_20260813T133538Z");
    expect(currentAcceptedPushedState.local_bundle).toBe("Accessibility and Saved-State Resilience QA Failure Repair");
    expect(currentAcceptedPushedState.local_bundle_phase).toBe("KIA-Stick-accessibility-and-saved-state-resilience-QA-failure-repair");
    ...
    expect(currentAcceptedPushedState.local_bundle_status).toBe("accessibility and saved-state resilience QA failure repair; validation PASS; pushed no; manual QA PASS; closeout and push authorized; post-push accepted-state refresh remains separately gated");
