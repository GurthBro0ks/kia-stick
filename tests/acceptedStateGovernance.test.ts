...
describe("accepted-state governance loop break", () => {
  it("records the pushed Bundle 3 core repairs as current while retaining the prior Bundle 3 capability as history", () => {
    ...
    expect(currentAcceptedPushedState.local_implementation_proof_dir).toBe("/home/mint/kia-stick-local-proofs/proof_kia_stick_accessibility_saved_state_qa_failure_repair_20260813T124812Z");
    ...
    expect(currentAcceptedPushedState.local_bundle_status).toBe("accessibility and saved-state resilience QA failure repair; validation PASS; pushed no; manual QA pending operator review; closeout and push await separate explicit authorization");
