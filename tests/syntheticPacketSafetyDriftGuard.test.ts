...
describe("v0.7.16 synthetic packet safety drift guard", () => {
  ...
  it("tracks package, version, and queue state without changing accepted report-runner state", () => {
    ...
    expect(featureList.release_readiness.status).toBe("v0716_synthetic_packet_safety_drift_guard_needs_review");
    ...
    expect(featureList.v0716_synthetic_packet_safety_drift_guard.status).toBe("validation_pass_needs_operator_review");
    ...
    expect(featureList.v0716_synthetic_packet_safety_drift_guard.queue_031_status).toBe("needs_review");
    ...
    expect(featureList.v0716_synthetic_packet_safety_drift_guard.push_performed).toBe(false);
    ...
    expect(q031?.status).toBe("needs_review");
