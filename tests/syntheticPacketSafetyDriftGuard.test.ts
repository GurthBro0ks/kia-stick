...
describe("v0.7.16 synthetic packet safety drift guard", () => {
  ...
  it("tracks package, version, and queue state without changing accepted report-runner state", () => {
    ...
    expect(featureList.v0716_synthetic_packet_safety_drift_guard.status).toBe("ready_to_push_after_operator_bundle_qa_pass");
    ...
    expect(featureList.v0716_synthetic_packet_safety_drift_guard.queue_031_status).toBe("ready_to_push");
    ...
    expect(q031?.status).toBe("ready_to_push");
