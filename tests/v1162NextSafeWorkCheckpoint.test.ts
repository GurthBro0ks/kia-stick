...
describe("v1.1.62 next safe work checkpoint", () => {
  it("keeps the new local checkpoint unpushed and all blocked states closed", () => {
    const state = JSON.parse(readFileSync("feature_list.json", "utf8")).v1162_next_safe_work_checkpoint;
    expect(state.current_accepted_pushed_short_commit).toBe("65f8865");
