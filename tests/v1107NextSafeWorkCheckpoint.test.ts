...
const phase = "KIA-Stick-v1.1.7-next-safe-work-checkpoint";
const docPath = "docs/v1.1.7-next-safe-work-checkpoint.md";

describe("v1.1.7 next safe work checkpoint", () => {
  ...
  it("tracks the checkpoint without package, queue, or real-doc drift", () => {
    const state = JSON.parse(readFileSync("feature_list.json", "utf8")).v1107_next_safe_work_checkpoint;
    expect(state.phase).toBe(phase);
    expect(state.status).toBe("accepted");
    expect(state.current_accepted_pushed_short_commit).toBe("6d0715b");
