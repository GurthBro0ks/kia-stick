...
describe("v1.1.72 next safe work checkpoint", () => {
  it("keeps the new local checkpoint unpushed and all blocked states closed", () => {
    const state = JSON.parse(readFileSync("feature_list.json", "utf8")).v1172_next_safe_work_checkpoint;
    expect(state.current_accepted_pushed_short_commit).toBe("0cb007d");
