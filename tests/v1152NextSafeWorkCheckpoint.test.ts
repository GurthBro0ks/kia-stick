...
describe("v1.1.52 next safe work checkpoint", () => {
  it("keeps the refresh local-only and blocked states intact", () => {
    const state = JSON.parse(readFileSync("feature_list.json", "utf8")).v1152_next_safe_work_checkpoint;
    expect(state.current_accepted_pushed_short_commit).toBe("b8fb834");
