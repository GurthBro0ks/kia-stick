...
const phase = "KIA-Stick-v1.0.7-next-safe-work-checkpoint";
const docPath = "docs/v1.0.7-next-safe-work-checkpoint.md";

describe("v1.0.7 next safe work checkpoint", () => {
  ...
  it("tracks the checkpoint without package, queue, or real-doc drift", () => {
    const featureList = JSON.parse(readFileSync("feature_list.json", "utf8")) as {
      v1007_next_safe_work_checkpoint: {
        phase: string;
        status: string;
    ...
    const state = featureList.v1007_next_safe_work_checkpoint;

    expect(state.phase).toBe(phase);
    expect(state.status).toBe("accepted");
    expect(state.current_accepted_pushed_short_commit).toBe("80e91c7");
    ...
    expect(state.operator_qa_pass_text).toBe(
      "OPERATOR_QA_PASS for /home/mint/kia-stick-local-proofs/proof_kia_stick_v1_0_3_to_v1_0_7_post_closeout_accepted_state_contract_refresh_20260702T152801Z"
    );
