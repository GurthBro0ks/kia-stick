...
describe("v0.7.17 synthetic packet fixture matrix", () => {
  ...
  it("keeps feature and queue state review-only without changing product or prompt identity", () => {
    ...
    expect(featureList.v0717_synthetic_packet_fixture_matrix.status).toBe("ready_to_push_after_operator_bundle_qa_pass");
    expect(featureList.v0717_synthetic_packet_fixture_matrix.queue_015_status).toBe("blocked");
    expect(featureList.v0717_synthetic_packet_fixture_matrix.queue_032_status).toBe("ready_to_push");
    ...
    expect(q032?.status).toBe("ready_to_push");
