...
describe("v0.7.17 synthetic packet fixture matrix", () => {
  ...
  it("keeps feature and queue state review-only without changing product or prompt identity", () => {
    ...
    expect(featureList.v0717_synthetic_packet_fixture_matrix.status).toBe("ready_for_operator_bundle_review");
    expect(featureList.v0717_synthetic_packet_fixture_matrix.queue_015_status).toBe("blocked");
    expect(featureList.v0717_synthetic_packet_fixture_matrix.queue_032_status).toBe("needs_review");
    ...
    expect(q032?.status).toBe("needs_review");
